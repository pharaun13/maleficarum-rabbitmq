...
use Maleficarum\Rabbitmq\Injector\ContextTrackerHeaderInjector;
...
class Manager
{
    ...
    public function addConnection(
        \Maleficarum\Rabbitmq\Connection\Connection $connection,
        string $identifier,
        int $mode,
        int $priority = 0
    ): \Maleficarum\Rabbitmq\Manager\Manager {
        // check if mode does not indicate both transient and persistent connection at the same time
        if (($mode & self::CON_MODE_PERSISTENT) && ($mode & self::CON_MODE_TRANSIENT)) {
            throw new \InvalidArgumentException(
                sprintf(
                    'Invalid connection mode specified - the connection cannot be both transient and persistent. %s()',
                    __METHOD__
                )
            );
        }
        ...
        if (!($mode & self::CON_MODE_PERSISTENT) && !($mode & self::CON_MODE_TRANSIENT)) {
            throw new \InvalidArgumentException(
                sprintf(
                    'Invalid connection mode specified - the connection cannot be neither transient nor persistent. %s()',
                    __METHOD__
                )
            );
        }
        ...
        if (array_key_exists($identifier, $this->connections)) {
            throw new \InvalidArgumentException(sprintf('Duplicate connection identifier. %s()', __METHOD__));
        }

        // validate priority value
        if ($priority < 0) {
            throw new \InvalidArgumentException(
                sprintf('Invalid connection priority - non-negative integer expected. %s', __METHOD__)
            );
        }
    ...
    public function removeConnection(string $connectionIdentifier): \Maleficarum\Rabbitmq\Manager\Manager
    {
        ...
        if (!array_key_exists($connectionIdentifier, $this->connections)) {
            throw new \InvalidArgumentException(
                sprintf('Provided connection identifier does not exist. %s', __METHOD__)
            );
        }
    ...
    public function addCommand(
        \Maleficarum\Command\AbstractCommand $command,
        string $connectionIdentifier,
        array $commandHeaders = []
    ): \Maleficarum\Rabbitmq\Manager\Manager {
        ...
        $commandHeaders = $this->injectHeaders($commandHeaders);
    ...
    public function addCommands(
        array $commands,
        string $connectionIdentifier,
        array $commandsHeaders = []
    ): \Maleficarum\Rabbitmq\Manager\Manager {
        // validate commands - set count
        if (count($commands) < 1) {
            throw new \InvalidArgumentException(sprintf('Expected a nonempty array of commands. \%s()', __METHOD__));
        }
        ...
        // send commands
        $channel = $connection->getChannel();
        $commandsHeaders = $this->injectHeaders($commandsHeaders);
    ...
    public function addRawMessage(string $message, string $connectionIdentifier): \Maleficarum\Rabbitmq\Manager\Manager
    {
        ...
        $commandsHeaders = $this->injectHeaders([]);
        $applicationHeaders = \Maleficarum\Ioc\Container::get('PhpAmqpLib\Wire\AMQPTable', [$commandsHeaders]);
    ...
    public function fetchSources(): array
    {
        ...
        foreach ($this->sources as $connectionIdentifier) {
            array_key_exists(
                $this->connections[$connectionIdentifier]['priority'],
                $sources
            ) or $sources[$this->connections[$connectionIdentifier]['priority']] = [];
    ...
    public function fetchSource(string $connectionIdentifier): \Maleficarum\Rabbitmq\Connection\Connection
    {
        ...
        if (!array_key_exists($connectionIdentifier, $this->sources)) {
            throw new \InvalidArgumentException(
                sprintf(
                    'Provided connection identifier matched a connection that cannot be used as a command source. %s',
                    __METHOD__
                )
            );
        }
     ...
     * @return string
     */
    private function getConnectionIdentifier(
        \Maleficarum\Command\AbstractCommand $command,
        string $connectionIdentifier
    ): string {
    ...
    /**
     * @param array<string, string> $commandHeaders
     * @return array<string, string>
     */
    protected function injectHeaders(array $commandHeaders): array
    {
        $headerInjectors = $this->getHeaderInjectors();
        foreach ($headerInjectors as $injector) {
            $commandHeaders = $injector->inject($commandHeaders);
        }
        return $commandHeaders;
    }

    /**
     * @return ContextTrackerHeaderInjector[]
     */
    protected function getHeaderInjectors(): array
    {
        $defaultInjectors = [new ContextTrackerHeaderInjector()];
        return array_merge($this->headerInjections, $defaultInjectors);
    }
